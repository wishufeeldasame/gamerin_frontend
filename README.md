...
* **26/09/03** 김신의
  ...
  > `community-search-api.ts`의 해시태그 자동완성, 태그별 게시글, 통합 검색, 탭별 검색 함수에 `AbortSignal` 옵션을 추가
...
* **26/08/08** 김신의
  ...
  > `src/lib/community-search-api.ts`를 추가하여 해시태그 자동완성, 태그별 게시글 조회, 통합 검색, 계정/게시글/해시태그 탭별 검색 API 호출을 분리
...
* **26/08/21** 전준범
  ...
  > `/admin/login`과 관리자 전용 레이아웃을 구성하고, `ADMIN`·`ROLE_ADMIN` 역할을 동일한 관리자 권한으로 판정하도록 정리
  > `/admin/login`, `/admin/forbidden`을 제외한 관리자 경로에 인증·권한 가드를 적용하고, 권한이 없으면 로그인 또는 권한 없음 화면으로 이동하도록 구현
  > 관리자 로그인 후 `/auth/me`로 권한과 계정 상태를 다시 확인하고, 로그아웃 시 서버 세션과 프론트 인증 정보를 함께 정리하도록 연결
  > 관리자 API의 `401 Unauthorized`는 세션 정리 후 로그인 화면으로, `403 Forbidden`은 세션을 유지한 채 권한 없음 화면으로 이동하도록 처리
  ...
  > 백엔드 API가 준비되지 않은 사용자 제재, 콘텐츠 복구, 감사 로그, 멘토 승인, 시스템 설정 기능은 데모임을 표시하고 변경 버튼을 비활성화
...
* **26/09/07** 전준범
  ...
  > 로그아웃 시작과 동시에 메모리 토큰·로컬 사용자·`AuthContext`를 삭제하고, 서버 로그아웃은 5초 제한과 단일 요청으로 처리해 지연·실패·중복 요청에 대응
  > `storage` 이벤트를 이용해 한 탭에서 로그아웃하면 다른 관리자 탭도 인증 정보를 삭제하고 로그인 화면으로 이동하도록 동기화
  > 관리자 API의 `401`은 세션을 완전히 삭제한 뒤 로그인 화면으로 이동하고, `403`은 세션을 유지한 채 권한 없음 화면으로 이동하도록 정리
  > 백엔드 제공 범위에 맞춰 관리자 신고 목록 조회·검색·필터링과 상태 변경 API를 연결하고, 상태 변경 후 현재 조건으로 목록·전체 건수·페이지 정보를 다시 조회
  > 신고 사유 필터를 신고 사유 조회 API의 `code`·`label`로 동적 생성하고, 조회 실패 시 신고 목록은 유지하면서 사유 필터만 비활성화하고 재시도할 수 있도록 구현
  ...
  > 모든 일반 신고 사유에서 상세 내용을 선택적으로 입력할 수 있도록 개선하고, `OTHER`는 필수 입력, 공백 입력은 `null` 전송, 300자 제한과 중복 제출 방지를 적용
...
* **26/09/18** 전준범
  ...
  > [B-1/P2] 북마크 해제에 성공해 게시글이 목록에서 제거된 뒤에도 모음집의 `bookmarkCount`가 이전 값으로 남아 있던 상태 불일치 문제 수정
  > `unbookmarkPost()` 성공 후 기존 `onBookmarkSuccess` 후처리를 먼저 실행하고, `refreshCollections()`로 서버의 최신 모음집 목록과 개수를 다시 조회하도록 연결
  > `unbookmarkPost()` 실패와 `refreshCollections()` 실패를 분리하여 DELETE 요청이 실패한 경우에만 기존 북마크 상태로 롤백하도록 처리
  > DELETE 성공 후 모음집 재조회가 실패하더라도 성공한 북마크 해제와 게시글 목록 제거를 되돌리거나 추가 오류 알림을 표시하지 않도록 방어 처리
  > 모음집 재조회가 완료되기 전에는 이전 개수를 다시 노출하지 않고 `동기화 중`을 표시하며, 새 컬렉션 배열이 반영된 뒤 로컬 개수 override를 초기화하도록 구현
  > 백엔드 DELETE가 해당 게시물의 모든 북마크 모음집 연결을 제거하는 계약을 확인하여 재조회 완료 후 전체 override를 정리하도록 적용
  > 기존 optimistic update, 북마크 추가, `skipRequest`, API 오류 문구와 `BookmarkCollectionContext` 공개 인터페이스는 그대로 유지
...
* **26/09/23** 전준범
  ...
  > [GitHub 이슈 #52] 게시글 상세 화면에서도 북마크 전체 해제와 마지막 컬렉션 체크 해제 성공 후 `refreshCollections()`를 호출해 `/bookmarks`의 컬렉션 개수를 서버 기준으로 갱신
  > 북마크 해제 API 실패 시 상세 화면의 북마크 상태를 이전 값으로 롤백하고 기존 컬렉션 목록은 유지하도록 처리
  > 컬렉션 재조회 실패 시 이미 성공한 북마크 해제는 유지하고, 북마크 추가 및 `skipRequest` 경로에서는 중복 재조회를 하지 않는 기존 정책 유지
  > 초기 컬렉션 조회가 끝나기 전에는 전체 해제를 비활성화하여 늦게 도착한 이전 응답이 최신 컬렉션 개수를 덮는 경합 방지
  > 상세 북마크 버튼에 상태별 접근성 이름을 추가하고, 실제 `SaveToCollectionModal`을 연결한 회귀 테스트로 두 해제 방식과 실패·추가 경로를 검증
  > 검증: 전체 단위 테스트 82건, `npm run lint`, `npx tsc --noEmit`, `npm run build`, `git diff --check` 통과

  > 요약 : 게시글 상세의 북마크 해제 후 컬렉션 개수 동기화를 보완하여 이슈 #52의 모든 화면 조건을 완료

* **26/09/24** 전준범

  > [GitHub 이슈 #54] 좋아요 요청 실패 시 클릭 당시 게시글 객체 전체를 복원해 요청 중 변경된 북마크·리포스트 상태까지 되돌아가던 문제 수정
  > 최신 게시글 배열을 기준으로 `likedByMe`와 `likes`만 변경하는 `updatePostsLikeState()` 공용 헬퍼를 `post-mutations.ts`에 추가
  > 홈·검색·해시태그·프로필의 낙관적 좋아요 갱신과 실패 롤백을 함수형 상태 갱신으로 통일하여 북마크·컬렉션·리포스트 최신 상태를 유지
  > 검색 화면은 전체 탭의 `overview.posts.items`와 게시글 탭의 `posts`를 계속 함께 갱신하도록 기존 동기화 동작 유지
  > 좋아요 요청을 지연시킨 동안 북마크·리포스트를 변경한 뒤 요청을 실패시키는 회귀 테스트를 네 화면에 추가하고, 공용 헬퍼 계산과 필드 보존 단위 테스트를 구성
  > #52 병합 후 기존 게시글 상세 deep-link 테스트가 `BookmarkCollectionContext` 없이 실패하던 테스트 환경을 mock으로 보완
  > 백엔드, API 계약, `feed-api.ts`, 컴포넌트 props와 기존 오류 문구는 변경하지 않음
  > 검증: 전체 단위 테스트 36개 파일·252건, `npm run test`, `npm run lint`, `npx tsc --noEmit`, `npm run build`, `git diff --check` 통과

  > 요약 : 좋아요 실패 시 좋아요 필드만 원복하고 요청 중 성공한 북마크·리포스트 상태를 유지하도록 네 게시글 목록 화면의 상태 갱신과 회귀 테스트를 보강
