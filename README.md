...
- **26/07/10** 김신의
  ...
  > 프로필 상단 소개글 영역에 대표 게임 전적을 표시하는 `ProfileStatsSummary` 컴포넌트 추가
  > `RepresentativeStat` 타입을 정의하고 게임명, 티어, K/D, 승률, 플레이 수, 티어 이미지 URL을 받을 수 있도록 구성
  > 프로필 API의 `gameStats`가 객체 또는 배열 형태로 내려와도 첫 번째 유효 전적을 대표 전적으로 파싱하도록 매핑 로직 보완
  > 하단 `Verified Stats` 탭에서 사용 중인 `kda`, `winRate`, `games` 필드를 상단 대표 전적 요약에도 재사용하도록 연결
  > 대표 전적 데이터가 없을 경우 기존 소개글 또는 기본 문구가 그대로 표시되도록 fallback 처리
  > 기존 회색 카드 UI를 제거하고 소개글 자리에 자연스럽게 녹아드는 한 줄 요약 UI로 변경
  > 백엔드는 향후 대표 전적 필드를 별도로 제공할 경우 `gameName`, `tier`, `kd/kda`, `winRate`, `playCount/games`, `tierImageUrl` 형태로 내려주면 프론트 매핑 가능
  > 검증: `npm run lint`, `tsc --noEmit` 통과
