...
GitHub Copilot 리뷰에서 지적될 수 있는 검색/해시태그 연동 안정성 이슈를 사전 보완
검색어 변경, 탭 전환, 해시태그 페이지 이동 시 이전 요청이 늦게 도착해 최신 화면 상태를 덮어쓰지 않도록 AbortController 흐름을 API 호출까지 연결
community-search-api.ts의 해시태그 자동완성, 태그별 게시글, 통합 검색, 탭별 검색 함수에 AbortSignal 옵션을 추가
게시글 작성 textarea의 해시태그 자동완성 요청도 cleanup 시 abort되도록 수정하여 빠른 입력 중 stale suggestion 표시 가능성을 줄임
해시태그 상세 화면에서 잘못 인코딩된 경로 값이 들어와도 렌더링이 깨지지 않도록 디코딩 방어 처리 추가
검색 결과 화면과 해시태그 화면의 한글 문구, 탭 라벨, 더보기 문구를 최신 UI 기준으로 정리
검증: npm.cmd run lint, npm.cmd run build, git diff --check 통과

요약 : 검색/해시태그 API 요청 취소 처리와 화면 문구를 보완하여 리뷰 전 안정성을 정리

26/08/08 김신의

백엔드 해시태그 및 통합 검색 API 계약에 맞춰 프론트 검색 화면을 실제 API 연동 방식으로 전환
src/lib/community-search-api.ts를 추가하여 해시태그 자동완성, 태그별 게시글 조회, 통합 검색, 계정/게시글/해시태그 탭별 검색 API 호출을 분리
게시글 작성 textarea에서 #태그 입력 중 해시태그 자동완성 목록을 표시하고 선택 시 본문에 #태그 형태로 삽입되도록 구현
게시글 목록과 상세 화면의 본문에서 한글, 영문, 숫자, 언더스코어 해시태그를 클릭 가능한 링크로 렌더링
/hashtags/[name] 화면을 추가하여 해시태그별 게시글을 최신순 커서 기반으로 조회하고 더보기로 다음 페이지를 불러오도록 구현
/search 화면을 전체/계정/게시글/해시태그 탭 구조로 정리하고, 전체 탭은 preview 검색 결과, 계정/게시글 탭은 커서 기반 더보기, 해시태그 탭은 목록 조회 방식으로 연동
검색어 trim 및 1~100자 검증, 해시태그 path parameter encodeURIComponent 처리, 인증 토큰 포함 API 호출 흐름 반영
최신 브랜치 기준 기존 북마크 화면 타입 오류와 메시지 SSE 재연결 옵션 타입 오류를 함께 보정하여 빌드 가능 상태로 정리
검증: npm.cmd run lint, npm.cmd run build 통과

요약 : 해시태그 자동완성, 해시태그 링크, 태그별 게시글 화면, 통합 검색 탭 화면을 백엔드 develop 최신 계약에 맞춰 연동
...
AuthContext에 isAuthReady와 옵션형 logout()을 추가하여 인증 초기화 및 로그아웃 흐름 정리
로그아웃 시 /login으로 즉시 이동하도록 수정하고 /home 및 하위 경로에 로그인 가드 적용
로그인 페이지에서 이미 로그인된 사용자는 /home으로 리다이렉트되도록 수정
...
주요 이미지 컴포넌트를 next/image로 전환하고 next.config.ts에 Unsplash 원격 이미지 허용 설정 추가
useSearchParams()를 사용하는 /auth/reset-password, /find-id-result 페이지에 Suspense 경계 추가
Header, Sidebar의 로그아웃 핸들러 타입 충돌 및 AddAcountModal import 경로 오류 수정
...
auth-store에 AUTH_USER_KEY, AUTH_CLEARED_EVENT, clearStoredAuth()를 추가하여 인증 상태 정리 로직 통합
refresh token 검증 실패 시 gamerin_access_token뿐 아니라 gamerin_user도 함께 삭제하도록 수정
인증 상태가 정리되면 전역 이벤트를 발생시켜 AuthContext의 user 상태도 즉시 null로 동기화
앱 부팅 시 저장된 gamerin_user를 바로 로그인 상태로 처리하지 않고 refreshAccessToken()으로 서버 세션 유효성 먼저 검증
refresh 실패 또는 저장된 유저 정보 파싱 실패 시 저장된 인증 정보를 정리하고 /home 진입 대신 /login으로 이동되도록 처리
login, updateUser, logout에서 gamerin_user 문자열 직접 사용을 제거하고 공통 인증 저장소 상수를 사용하도록 수정
...
Docker 빌드 시 NEXT_PUBLIC_API_BASE_URL을 build arg로 주입하도록 구성
reverse proxy 환경에서 NEXT_PUBLIC_API_BASE_URL을 비워 /api/... 상대경로 호출이 가능하도록 구성
production 컨테이너에서 npm run start로 Next.js 앱을 3000 포트에서 실행하도록 구성
...
PostDetail에서 게시글 상세 정보와 댓글 목록을 함께 요청하도록 변경
...
Sidebar의 북마크 메뉴 경로를 /home/bookmarks로 수정
fetchMyBookmarks()를 사용해 내 북마크 목록을 커서 기반으로 불러오고 더 보기 버튼으로 다음 페이지를 조회하도록 구현
기존 bookmark-store.ts와 북마크 localStorage 이벤트 의존성을 제거하여 서버 데이터를 단일 기준으로 정리
...
리뷰 작성 성공 또는 중복 리뷰 방어 처리 시 해당 신청 항목의 reviewed 값을 로컬 상태에서도 즉시 true로 갱신하도록 보강
MentoringApplicationResponse 타입에 reviewed를 추가하고, 백엔드 응답 계약에 맞춰 mentorId, menteeId, programTitle 사용 흐름을 정리
실제 백엔드에 없는 신청 삭제 API 호출 함수 deleteMentoringApplication()을 제거하고, 취소된 신청은 CANCELLED 상태 배지만 표시하도록 UI 정리
404 응답 메시지 처리에서 message가 없을 수 있는 타입 오류를 방지하도록 MentoringApiError 생성 로직 보완

26/06/06 서장호

Docker/nginx 배포 환경에서 메시지 API가 localhost:8080으로 직접 호출되어 ERR_CONNECTION_REFUSED가 발생하던 문제 수정
src/lib/api-base.ts에서 NEXT_PUBLIC_API_BASE_URL이 비어 있을 때 브라우저의 window.location.origin을 API base로 사용하도록 변경
...
요약 : 메시지 API가 localhost:8080으로 직접 호출되던 문제를 수정하고, nginx reverse proxy 기준으로 메시지/SSE 연결이 동작하도록 정리

26/06/06 서장호

백엔드에서 메시지 수정 API를 지원하지 않도록 정리한 계약에 맞춰 메시지 수정 버튼과 편집 입력 UI 제거
src/lib/message-api.ts에서 updateConversationMessage()와 message-updated 실시간 이벤트 타입 제거
src/lib/message-store.ts와 메시지 응답 변환 흐름에서 editedAt 필드 제거
...
/messages는 대화 미선택 상태로 유지하고, 대화 카드 클릭 또는 conversationId/recipient 쿼리 기반 명시 진입만 대화방을 열도록 정리
...
src/app/(app)/layout.tsx에 main 브랜치의 소셜 로그인 후 무한 리다이렉트 방지 로직을 반영하고, 인증 준비 전/비로그인 상태에서 보호 화면이 렌더링되지 않도록 가드 유지
src/app/(app)/home/page.tsx는 PR 브랜치의 /posts/[postId] 상세 라우팅 구조를 유지하도록 충돌 해결
...
백엔드 PATCH /api/v1/users/me가 성공 시 프로필 객체가 아닌 data: null을 반환하는 계약에 맞춰 updateMyProfile() 흐름 수정
프로필 수정 성공 후 GET /api/v1/users/me를 다시 호출해 최신 프로필을 화면과 인증 사용자 상태에 반영하도록 변경
UserProfile 타입에 백엔드 응답 필드인 location, website, coverImageUrl을 추가
편집 모달의 location, website 초기값을 서버 프로필 값으로 채우고, 저장 시 nickname, bio, location, website를 백엔드 프로필 수정 API로 전송하도록 연결
...
프로필 화면에 위치와 웹사이트 표시를 추가하고, 다른 사용자 프로필에서도 서버 coverImageUrl을 사용하도록 수정
...
인증 사용자 캐시에 profileImageUrl을 유지하도록 정규화 흐름을 보완
...
BookmarkCollectionProvider를 통해 컬렉션 생성, 게시글 저장, 저장 해제 상태를 전역으로 관리하도록 구현
백엔드 컬렉션 API가 준비되기 전까지 localStorage의 gamerin_bookmark_collections 키를 사용해 컬렉션 상태를 임시 저장하도록 처리
피드와 게시글 상세 화면의 북마크 버튼 클릭 시 SaveToCollectionModal이 열리도록 연결
...
feature/post-repost가 feature/bookmark-collections에서 분기된 상태라 PR 병합 시 북마크 컬렉션 브랜치 선병합 또는 리포스트 커밋 재배치가 필요함을 정리
...
GameFilter를 재사용 가능한 제어 컴포넌트로 분리하고 전체, PUBG, VALORANT, LEAGUE OF LEGENDS, FPS 필터를 제공하도록 구성
...
feature/global-search-explore가 feature/bookmark-collections에서 분기된 상태라 PR 병합 시 북마크 컬렉션 브랜치 선병합 또는 검색 기능 커밋 재배치가 필요함을 정리
검증: npm run lint, tsc --noEmit 통과
...
SSE 연결 전에 POST /api/v1/messages/stream-token을 호출하여 HttpOnly 전용 쿠키를 발급받도록 변경
토큰 없는 /api/v1/messages/stream URL에 withCredentials: true로 연결하도록 변경
...
이전 공용 키 gamerin_bookmark_collections는 읽지 않고 삭제하여 계정 간 모음집 정보가 다시 표시되지 않도록 정리
모음집에 게시글을 추가하거나 제거할 때 서버 북마크 요청이 성공한 뒤에만 로컬 savedPostIds를 변경하도록 수정
...
GET /api/v1/bookmark-collections와 GET /api/v1/bookmark-collections?postId={postId}를 사용해 모음집 목록 및 게시글별 저장 상태를 조회하도록 연결
모음집 생성 시 POST /api/v1/bookmark-collections에 name, initialPostId를 전송하여 생성과 동시에 현재 게시글을 저장하도록 처리
...
/bookmarks 페이지 조회를 서버 기준으로 변경하여 전체는 scope=all, 미분류는 scope=unclassified, 모음집별은 /api/v1/bookmark-collections/{collectionId}/bookmarks를 사용하도록 연결
검색어 q, 미디어 필터 mediaOnly, 커서 cursor, 페이지 크기 size를 백엔드 쿼리 파라미터로 전달하도록 변경
컬렉션 목록의 bookmarkCount와 실제 조회 결과가 일시적으로 어긋나는 경우 선택된 모음집 조회 결과 기준으로 화면 카운트를 보정하도록 처리
...
auth-store.ts에 authGeneration을 추가하고 로그인·로그아웃 및 refresh 요청을 인증 세대별로 구분
feed-api.ts에서 요청 시작, 최초 응답, 401 refresh, 재시도 전후의 인증 세대를 검증하고 계정이 바뀌면 AbortError로 중단
BookmarkCollectionContext.tsx의 모음집 상태에 ownerId를 부여하고, 사용자 변경 시 기존 목록과 진행 중인 요청 상태를 즉시 초기화
모음집 조회·생성·추가·제거 응답은 요청 당시 사용자 ID와 인증 세대가 현재 사용자와 같은 경우에만 상태에 반영
AuthContext.tsx의 앱 시작 인증 복원에도 세대 검증을 적용하여 이전 bootstrap 응답이 새 로그인 상태를 덮지 않도록 처리
...
적용 범위: 북마크가 사용하는 feed-api.ts에 적용했으며 메시지·멘토링·마일리지 API는 별도 후속 작업으로 분리

요약 : 계정 전환 후 이전 계정의 북마크 응답과 401 재시도가 현재 계정의 토큰·모음집 상태를 덮지 못하도록 수정

26/07/21 서장호

PUBG와 Rainbow Six Siege의 전적 API 타입과 인증 요청을 game-stats-api.ts로 분리
...
선택한 게임에 따라 DELETE /api/v1/pubg/disconnect 또는 DELETE /api/v1/r6/disconnect를 호출하고 성공한 게임 카드만 제거하도록 구현
...
백엔드 리포스트 계약에 맞춰 ReposterInfo.repostedAt과 RepostActionResponse 타입을 추가
POST /api/v1/posts/{postId}/reposts, DELETE /api/v1/posts/{postId}/reposts 응답의 isReposted, repostCount를 프론트 상태 보정에 사용하도록 수정
게시글 목록 카드에서 리포스트 표시 영역에 리포스트 시간을 함께 표시하고, 본인 게시글 리포스트 버튼을 비활성화하도록 처리
게시글 상세 화면 액션 영역에 리포스트 생성·취소 버튼과 카운트 표시를 추가하고, 상세 응답에서는 isReposted, repostCount만 사용하도록 연결
...
일반 사용자 신고 API 클라이언트를 src/lib/report-api.ts로 분리
GET /api/v1/reports/reasons를 호출해 백엔드 신고 사유 코드와 라벨을 모달에 동적으로 표시
POST /api/v1/reports에 targetType, targetId, reasonCode, details를 전송해 실제 신고가 DB에 접수되도록 연결
기존 인증 흐름과 동일하게 Bearer access token, 인증 세대 검증, 401 refresh 후 1회 재시도를 적용
게시글은 POST와 postId, 댓글은 COMMENT와 commentId, 사용자 프로필은 USER와 사용자 UUID를 전달하도록 연결
OTHER 선택 시 최대 300자의 상세 사유를 전송하고, 사유 조회 로딩·재시도·접수 오류 상태를 화면에 표시
신고 API 성공 후에만 접수 완료 화면을 표시하고, 동일 대상을 다시 신고한 경우 409 Conflict를 이미 신고한 대상입니다. 메시지로 처리
로컬 백엔드 feat/report와 실제 통신하여 사유 조회 200, 신고 접수 200, 동일 대상 재신고 409, localhost:3000 CORS 허용을 확인
...
/admin/login과 관리자 전용 레이아웃을 구성하고, ADMIN·ROLE_ADMIN 역할을 동일한 관리자 권한으로 판정하도록 정리
/admin/login, /admin/forbidden을 제외한 관리자 경로에 인증·권한 가드를 적용하고, 권한이 없으면 로그인 또는 권한 없음 화면으로 이동하도록 구현
관리자 로그인 후 /auth/me로 권한과 계정 상태를 다시 확인하고, 로그아웃 시 서버 세션과 프론트 인증 정보를 함께 정리하도록 연결
관리자 API의 401 Unauthorized는 세션 정리 후 로그인 화면으로, 403 Forbidden은 세션을 유지한 채 권한 없음 화면으로 이동하도록 처리
관리자 대시보드, 신고 목록, 신고 상세, 사용자 관리, 콘텐츠 관리, 감사 로그, 멘토링 관리, 시스템 설정 화면과 기본 동작 흐름을 구성
관리자 신고 목록·상세 조회, 검토 시작, 처리 완료·반려 요청을 백엔드 신고 API 클라이언트와 연결하고 중복 요청 방지 및 오류 상태를 처리
아직 백엔드 API가 준비되지 않은 사용자 제재, 콘텐츠 복구, 감사 로그, 멘토 승인, 시스템 설정 기능은 데모임을 표시하고 변경 버튼을 비활성화
API 기본 주소를 getApiBaseUrl() 기준으로 통일하고, Next.js 15.5.22 기준 CSP와 주요 보안 응답 헤더를 설정
Vitest와 Playwright 기반 관리자 로그인·라우트 가드·로그아웃·401·403 처리 테스트를 추가
검증: npm run lint, npx tsc --noEmit, 단위 테스트 33건, npm run build 통과. 관리자 E2E 3건은 단일 워커 실행에서 통과
현재 백엔드 API가 개발 중이므로 실제 연동 환경의 수동 테스트는 진행하지 않았으며, 백엔드 연동 완료 후 로그인·신고 조회·처리 기능을 직접 테스트하고 오류와 누락 사항을 보완할 예정

요약 : 관리자 콘솔 UI와 인증·신고 처리 흐름, 보안 및 자동화 테스트 기반을 구현했으며, 백엔드 연동 후 수동 테스트를 거쳐 최종 완성 예정

26/09/07 전준범

Next.js 15.5.22 기준으로 CSP와 보안 응답 헤더를 정비하고, Vitest·Playwright 테스트 환경 및 테스트 산출물 제외 설정을 추가
관리자 로그인 시 /auth/me 결과로 ADMIN·ROLE_ADMIN 권한과 ACTIVE 계정 상태를 검증하고, SUSPENDED·DELETED 등 차단 계정은 로그인되지 않도록 처리
로그아웃 시작과 동시에 메모리 토큰·로컬 사용자·AuthContext를 삭제하고, 서버 로그아웃은 5초 제한과 단일 요청으로 처리해 지연·실패·중복 요청에 대응
storage 이벤트를 이용해 한 탭에서 로그아웃하면 다른 관리자 탭도 인증 정보를 삭제하고 로그인 화면으로 이동하도록 동기화
관리자 API의 401은 세션을 완전히 삭제한 뒤 로그인 화면으로 이동하고, 403은 세션을 유지한 채 권한 없음 화면으로 이동하도록 정리
백엔드 제공 범위에 맞춰 관리자 신고 목록 조회·검색·필터링과 상태 변경 API를 연결하고, 상태 변경 후 현재 조건으로 목록·전체 건수·페이지 정보를 다시 조회
신고 사유 필터를 신고 사유 조회 API의 code·label로 동적 생성하고, 조회 실패 시 신고 목록은 유지하면서 사유 필터만 비활성화하고 재시도할 수 있도록 구현
신고 목록, 대시보드 최근 신고, 숨김 콘텐츠를 30초마다 자동 갱신하고, 백그라운드 탭에서는 중지한 뒤 화면 복귀 시 즉시 갱신하도록 구현
숨김 콘텐츠를 서버 기준 20건 페이지네이션의 통합 목록으로 변경하고, 현재 페이지 UUID 검색 및 POST·COMMENT 복구 API를 연결
모든 일반 신고 사유에서 상세 내용을 선택적으로 입력할 수 있도록 개선하고, OTHER는 필수 입력, 공백 입력은 null 전송, 300자 제한과 중복 제출 방지를 적용
사용자 제재, 감사 로그, 멘토 승인, 시스템 설정 등 아직 API가 없는 관리자 기능은 데모 안내를 표시하고 실제 상태 변경 버튼을 비활성화
커밋을 설정·인증·미연동 데모·관리자 신고/콘텐츠·일반 신고·테스트·문서의 7개 범주로 분리하여 변경 이력을 정리
검증: npm run lint, npx tsc --noEmit, 단위 테스트 63건, 관리자 E2E 6건, npm run build 통과

요약 : 관리자 인증과 세션 안정성을 강화하고, 백엔드 신고·숨김 콘텐츠 API 연동, 자동 갱신, 페이지 보정, 동적 필터와 테스트를 보강했으며 미연동 기능은 안전한 데모 상태로 구분