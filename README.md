...
## 2026-07-09 리포스트 프론트엔드 개발 노트

작업 브랜치: `feature/post-repost`

### 구현 내용

- 게시글 타입에 `isReposted`, `repostCount`, `reposterInfo`를 추가했습니다.
- 게시글 카드 액션 영역에 리포스트 버튼과 리포스트 수를 표시합니다.
- 리포스트된 피드 항목은 카드 상단에 `{nickname}님이 리포스트했습니다`를 표시합니다.
- `useRepost` 훅에서 낙관적 업데이트, 요청 중 중복 클릭 차단, 실패 시 롤백을 처리합니다.
- 홈, 북마크, 프로필 게시글 목록의 로컬 상태에 리포스트 변경을 즉시 반영합니다.
- 프론트 API 함수는 아래 임시 계약으로 작성되어 있습니다.

```text
POST   /api/v1/posts/{postId}/reposts
DELETE /api/v1/posts/{postId}/reposts
```

### 백엔드 구현 및 협의 필요사항

현재 백엔드에는 위 리포스트 API가 없으므로 버튼을 누르면 낙관적으로 변경된 뒤 API 실패로 원상 복구됩니다.

1. 리포스트 생성과 취소 API를 구현해야 합니다.
2. 두 API는 로그인 사용자를 기준으로 동작해야 하며 동일 요청이 반복되어도 카운트가 중복 증가하거나 감소하지 않도록 멱등성을 보장해야 합니다.
3. 성공 응답은 현재 프론트 구현 기준으로 아래처럼 `data: null`이어도 됩니다.
...
4. 피드, 게시글 상세, 사용자 게시글, 북마크 목록의 `Post` 응답에 다음 필드가 필요합니다.
...
```json
{
  ...
  "repostCount": 3,
  "reposterInfo": {
    "userId": "reposter-user-id",
    "nickname": "리포스트한 사용자"
  }
}
```

- `isReposted`: 현재 로그인 사용자가 원본 게시글을 리포스트했는지 여부
- `repostCount`: 원본 게시글의 전체 리포스트 수
- `reposterInfo`: 현재 피드 항목이 리포스트로 노출될 때 리포스트한 사용자 정보
- 일반 원본 게시글에는 `reposterInfo`를 `null`로 내려도 됩니다.

5. 리포스트 피드 항목에서도 `postId`는 API 토글 대상인 원본 게시글 ID로 유지하거나, 별도의 `originalPostId`를 추가하는 방식으로 계약을 확정해야 합니다.
6. 본인 게시글 리포스트 허용 여부, 원본 삭제 시 리포스트 처리, 리포스트 목록의 정렬 기준을 백엔드 정책으로 확정해야 합니다.
7. 프론트는 현재 API 응답 본문으로 카운트를 재보정하지 않습니다. 서버에서 최종 `repostCount`를 반환하도록 변경하면 프론트 훅도 해당 값을 사용하도록 후속 수정이 필요합니다.

### 현재 부족한 부분

- 백엔드 API 미구현으로 실제 저장 및 취소는 동작하지 않습니다.
- 새로고침 후 상태는 서버 피드 응답에 의존합니다.
- 게시글 상세 화면에는 아직 리포스트 버튼을 추가하지 않았습니다.
- 리포스트 사용자 목록 모달과 알림 연동은 구현하지 않았습니다.
- API 실패 메시지는 현재 버튼의 `title`에만 보관되며 별도 토스트 UI는 없습니다.

### 브랜치 의존성

`feature/post-repost`는 `feature/bookmark-collections`에서 분기했습니다. PR 병합 시 `feature/bookmark-collections`를 먼저 병합하거나 리포스트 커밋을 대상 브랜치에 재배치해야 합니다.
