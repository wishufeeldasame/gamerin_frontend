...
이 문서는 GamerIN 프론트엔드 저장소 루트에서 Claude Code가 작업할 때 적용하는 팀 지침이다. 이 저장소만 클론하거나 열어도 사용할 수 있도록 공통 작업 규칙을 포함한다.
...
- 다른 저장소와 연결된 변경은 접근 가능한 해당 저장소의 코드와 `CLAUDE.md`(있는 경우)를 확인한다. 저장소가 없으면 현재 저장소에서 가능한 작업을 진행하고, 교차 검증하지 못한 계약과 필요한 자료를 보고한다. 경로를 추측하거나 자동으로 클론하지 않는다.
